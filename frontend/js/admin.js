    ...
    if (menuItem.children) {
      ...
      for (let i = 0; i < menuData.length; i++) {
        ...
        if (otherMenuItem !== menuItem && otherMenuItem.expanded) {
          ...
          const otherLiElement = menuSection.querySelector(
            `li[data-id="${otherMenuItem.id}"]`
          );
    ...
    function findFirstUrl(menuItems) {
      for (var i = 0; i < menuItems.length; i++) {
        ...
        if (menuItem.url) {
          ...
          var liElement = menuSection.querySelector(
            'li[data-url="' + menuItem.url + '"]'
          );
          liElement.classList.add('selected');
          foundFirstUrl = true;
          break;
